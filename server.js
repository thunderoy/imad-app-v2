...
app.get('/get-comments/:project_id', function (req, res) {
   ...
   pool.query('SELECT comment.comment, "user".username FROM projects, comment, "user" WHERE project.id = $1 AND project.id = comment.project_id AND comment.user_id = "user".id ORDER BY comment.timestamp DESC', [req.params.project_id], function (err, result) {
      if (err) {
          res.status(500).send(err.toString());
      } else {
